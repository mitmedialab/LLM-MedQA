...
##########################################################################################
# Confidence - Low Accuracy AI

# read in data and check
dat_Conf_L <- read.csv("/Users/shruthishekar/Desktop/Survey 2/Survey 2 - Conf - Low Accuracy AI.csv")
str(dat_Conf_L)
head(dat_Conf_L)

# estimate models
mod_null <- lmer(Confidence ~ 1 + (1 | Participant.ID) + (1 | Question.ID), data = dat_Conf_L)
mod <- lmer(Confidence ~ 1 + Incorrect.Correct + (1 | Participant.ID) + (1 | Question.ID), data = dat_Conf_L)
