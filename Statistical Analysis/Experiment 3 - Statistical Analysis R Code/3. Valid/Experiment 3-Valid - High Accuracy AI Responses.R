...
dat_valid_3_H <- read.csv("/Users/shruthishekar/Documents/GitHub/LLM-MedQA/3) Organized Experiment Data/Experiment 3.csv")
dat_valid_3_H = dat_valid_3_H[dat_valid_3_H$Response.Source == "Valid - H", ] #Limit dataset to responses for this evaluation question type

str(dat_valid_3_H)
head(dat_valid_3_H)

# estimate models
mod_null <- lmer(Response.Scores ~ 1 + (1 | Participant.ID) + (1 | Question.ID), data = dat_valid_3_H)
mod <- lmer(Response.Scores ~ 1 + Random.Header + (1 | Participant.ID) + (1 | Question.ID), data = dat_valid_3_H)
