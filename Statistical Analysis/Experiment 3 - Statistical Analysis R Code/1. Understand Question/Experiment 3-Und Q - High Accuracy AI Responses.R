...
dat_und_q_3_H <- read.csv("/Users/shruthishekar/Documents/GitHub/LLM-MedQA/3) Organized Experiment Data/Experiment 3.csv")
dat_und_q_3_H = dat_und_q_3_H[dat_und_q_3_H$Response.Source == "Und_Q - H", ]#Limit dataset to responses for this evaluation question type

str(dat_und_q_3_H)
head(dat_und_q_3_H)

# estimate models
mod_null <- lmer(Response.Scores ~ 1 + (1 | Participant.ID) + (1 | Question.ID), data = dat_und_q_3_H)
mod <- lmer(Response.Scores ~ 1 + Random.Header + (1 | Participant.ID) + (1 | Question.ID), data = dat_und_q_3_H)
