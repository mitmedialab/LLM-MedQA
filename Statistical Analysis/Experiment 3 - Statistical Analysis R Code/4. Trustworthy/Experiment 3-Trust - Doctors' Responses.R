...
dat_trust_3_D <- read.csv("/Users/shruthishekar/Documents/GitHub/LLM-MedQA/3) Organized Experiment Data/Experiment 3.csv")
dat_trust_3_D = dat_trust_3_D[dat_trust_3_D$Response.Source == "Trust - D", ] #Limit dataset to responses for this evaluation question type

str(dat_trust_3_D)
head(dat_trust_3_D)

# estimate models
mod_null <- lmer(Response.Scores ~ 1 + (1 | Participant.ID) + (1 | Question.ID), data = dat_trust_3_D)
mod <- lmer(Response.Scores ~ 1 + Random.Header + (1 | Participant.ID) + (1 | Question.ID), data = dat_trust_3_D)
