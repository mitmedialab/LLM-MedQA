...
dat_satis_3_L <- read.csv("/Users/shruthishekar/Documents/GitHub/LLM-MedQA/3) Organized Experiment Data/Experiment 3.csv")
dat_satis_3_L = dat_satis_3_L[dat_satis_3_L$Response.Source == "Satis - L", ] #Limit dataset to responses for this evaluation question type

str(dat_satis_3_L)
head(dat_satis_3_L)

# estimate models
mod_null <- lmer(Response.Scores ~ 1 + (1 | Participant.ID) + (1 | Question.ID), data = dat_satis_3_L)
mod <- lmer(Response.Scores ~ 1 + Random.Header + (1 | Participant.ID) + (1 | Question.ID), data = dat_satis_3_L)
